...
def generator(i_sn, z, output_size, batch_size, h_dim):
    ...
    # Encoder Layer 0
    i_sn_1 = tf.reshape(i_sn, [-1, 64*64])
    e0 = tf.nn.relu(linear(i_sn_1, 200, 'g_e0'))

    # Encoder Layer 1
    e1 = tf.nn.tanh(linear(e0, 100, 'g_e1'))
    ...
    h0 = tf.nn.relu(gbn_0(linear(z, 1024, 'g_h0')))
    h0 = tf.concat([h0, y], 1)
    # Layer 1
    gbn_1 = batch_norm(name='gbn_1')
    h1 = tf.nn.relu(gbn_1(linear(h0, 64*2*16*16,'g_h1')))
    h1 = tf.reshape(h1, [batch_size, 16, 16, 128])
    h1 = conv_cond_concat(h1, yb)
    # Layer 2
    gbn_2 = batch_norm(name='gbn_2')
    h2 = tf.nn.relu(gbn_2(fsconv(filter_size, h1, [batch_size, 32, 32, 128], strides=strides, scope='g_h2', stddev=stddev)))
    h2 = conv_cond_concat(h2, yb)
    # Layer 3
    h3 = tf.nn.relu(fsconv(filter_size, h2, [batch_size, 64, 64, 1], strides=strides, scope='g_h3', stddev=stddev))

    h3 = conv_cond_concat(h3, yb)
    h4 = tf.concat([h3, i_sn], 3)
    # Filtered Image
    h4 = tf.nn.tanh(fsconv([5,5], h4, [batch_size, 64, 64, 1], strides=[1,1,1,1], scope='g_h4',stddev=stddev))
    return h4
    ...
    def train(self):
        with tf.Session() as sess:
            ...
            for step in range(self.num_steps):
                ...
                x = input_sample(self.batch_size, self.data_real)
