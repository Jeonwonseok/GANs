...
def generator(i_sn, z, output_size, batch_size, h_dim):
    ...
    # CNN for latent vector
    # Encoder Layer 0
    e0 = tf.nn.relu(conv(filter_size, i_sn, 128, strides=strides, scope='g_e0', stddev=stddev))

    # Encoder Layer 1
    ebn_1 = batch_norm(name='ebn_1')
    e1 = tf.nn.relu(ebn_1(conv(filter_size, e0, 256, strides=strides, scope='g_e1', stddev=stddev)))

    # Encoder Layer 2
    ebn_2 = batch_norm(name='ebn_2')
    e2 = tf.nn.relu(ebn_2(conv(filter_size, e1, 512, strides=strides, scope='g_e2', stddev=stddev)))

    # Encoder Layer 3
    ebn_3 = batch_norm(name='ebn_3')
    e3 = tf.nn.relu(ebn_3(conv(filter_size, e2, 1024, strides=strides, scope='g_e3', stddev=stddev)))

    # Encoder Layer 4
    e4 = tf.nn.tanh(linear(tf.reshape(e3, [-1, 1024*4*4]), 100, 'g_e4'))
    ...
    # Layer 3
    h3_w = fsconv(filter_size, h2, [batch_size, s, s, c_dim], strides=strides, scope='g_h3_w', stddev=stddev)
    h3_b = fsconv(filter_size, h2, [batch_size, s, s, c_dim], strides=strides, scope='g_h3_b', stddev=stddev)

    # Filtered Image
    h4 = tf.nn.tanh(tf.multiply(tf.subtract(i_sn, h3_b), h3_w))
    return h4
...
def discriminator(input):
    ...
    h3 = lrelu(dbn_3(conv(filter_size, h0, 1024, strides=strides, scope='d_h3', stddev=stddev)))
...
class GAN_MNIST(object):
    def __init__(self, num_steps, batch_size, log_every, anim_path):
        ...
        self.data_train = glob(os.path.join("./lena_train", self.input_fname))
    ...
    def _create_model(self, scope=None):
        ...
        self.loss_g = 0.001*tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D2_logits, labels=tf.ones_like(self.D2_logits))) + self.MSE
    ...
    def train(self):
        with tf.Session() as sess:
            ...
            for step in range(self.num_steps):
                ...
                x = input_sample(self.batch_size, self.data_real_lena)
    ...
    def _samples(self, sess):
        ...
        gen_lena_merge = np.concatenate([gen_lena.reshape([64,64]), self.lena_real], 1)
        gen_camera_merge = np.concatenate([gen_camera.reshape([64,64]), self.camera_real], 1)
